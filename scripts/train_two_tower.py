...
from src.models.two_tower_ranker import TwoTowerRanker, TwoTowerDataset


def train(args):
    print("Training Bi-TAML (Two-Tower)")
    ...
    model = TwoTowerRanker(TWO_TOWER_CONFIG, emb_sizes)
    ...
    results_file = os.path.join(
        args.results_dir, 
        f'two_tower_results_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
    )
    with open(results_file, 'w') as f:
        json.dump({
            'model': 'Bi-TAML Two-Tower',
...
def train_model(model, train_loader, val_loader, config):
    ...
    for epoch in range(config['epochs']):
        ...
        for batch in train_loader:
            ...
            time_shift_bin = batch['time_shift_bin'].to(DEVICE)
            news_age = batch['news_age'].to(DEVICE)
            labels = batch['label'].to(DEVICE)
            
            optimizer.zero_grad()
            preds = model(context, content_emb, category, time_shift_bin, news_age)
...
def evaluate_model(model, test_loader):
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            outputs = model(context, content_emb, category, time_shift_bin, news_age)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Train Bi-TAML Two-Tower model')
    ...
    parser.add_argument('--cluster_path', type=str, default=CLUSTER_PATH)
    parser.add_argument('--model_save_path', type=str, default=os.path.join(MODELS_DIR, 'two_tower_ranker.pth'))
